...
USER_HASHES = {
    ...
    "組長審查區不要亂點": "7141308b69010ec53a32ea3ccc3e9ac8efa14270b2a202f2e2df4da0ff220d87"  # 789
}
...
def get_drive_service():
    scopes = [
        ...
        'https://www.googleapis.com/auth/drive'  # 👈 修正1：已經幫她把 .file 刪除了
    ]
    try:
...
if role == "班級/導師查閱缺失點這邊":
    ...
    except: raw_df = pd.DataFrame(); current_week = 1
    ...
    else: instant_ranks = calculate_leader_stats(pd.DataFrame())
    ...
    if search_class != "請選擇":
        ...
        else: st.success(f"🌟 班級 {search_class} 在該週次無缺失紀錄。")
...
# --- B. 一般衛糾登錄 ---
    ...
    if verify_password(input_pw, role):
        ...
        with col_date: report_date = st.date_input("📅 補單日期", value=now_tw.date())
        with col_pid: p_id = st.selectbox("巡區編號 (必填)", ALL_PATROL_ZONES)
        ...
        # 項目選擇
        all_selected = st.multiselect("🚽 廁所", ["201 小便斗不乾淨", "202 大便池不乾淨", "203 洗手台不乾淨", "204 鏡子不明亮", "205 垃圾桶不乾淨", "2061 未補充衛生紙", "2062 未補充香皂", "2071 紙匣不乾淨", "2072 皂架不乾淨", "2073 感應器不乾淨", "2074 沖水器不乾淨", "5011 拖把槽入水孔不淨", "5012 拖把槽下方不淨", "502 地面未刷洗", "503 地面太濕", "504 隔間內地板髒", "701 垃圾未倒", "703 廁所門反鎖"]) + \
                       st.multiselect("掃區域", ["211 地面不乾淨", "212 地面未拖", "213 地面太濕", "215 灰塵堆積", "2161 窗台(內)髒", "2162 窗台(走道)髒", "218 扶手欄杆髒", "222 教室門板髒", "5101 黑板不乾淨", "5102 板溝不乾淨", "5111 講台桌髒", "5112 講台地髒", "5113 講台雜物", "5141 蜘蛛網(牆壁)", "5142 蜘蛛網(天花板)", "5151 玻璃(內側)", "5152 玻璃(走道側)", "5153 玻璃(門)", "518 飲水機不潔", "524 走廊堆放雜物"]) + \
                       st.multiselect("♻️ 掃具與垃圾", ["219 工具擺放不整齊", "220 回送桶放室外", "221 洗手槽廚餘未清", "512 掃具未貼標籤", "5161 垃圾箱外殼", "5162 垃圾箱內部", "5163 垃圾箱四格未齊", "523 垃圾箱內未放桶", "704 掃具隨意放置", "705 工具間髒亂", "1000 垃圾桶未倒", "1001 垃圾桶過過多未倒", "1002 紙類回收未倒", "1003 垃圾任意放置"]) + \
                       st.multiselect("⚠️ 重大違規", ["520 打掃遲到", "1004 沒打掃", "1005 嬉戲打球", "1006 態度不佳", "1007 惡意丟棄垃圾", "1008 玩手機(累計制)"])
        
        extra_late = ""; extra_phone = 1
        if any("520" in i for i in all_selected): extra_late = st.text_input("🕒 遲到時間")
        if any("1008" in i for i in all_selected): extra_phone = st.number_input("📱 玩手機人數", min_value=1, step=1)
            ...
            if target_class != "請選擇" and (all_selected or user_note):
                with st.spinner("雲端上傳中，請稍候..."):
                    ...
                    for item in all_selected:
                        ...
                        if code == "1008": f_list.append(f"{item} ({extra_phone}人)"); p_score += (1 * extra_phone)
                        elif code == "520": f_list.append(f"{item} ({extra_late})"); n_score += weight
                        else: f_list.append(item); n_score += weight
                    if user_note: f_list.append(f"備註: {user_note}")
                    ...
                    try:
                        ...
                        st.success("✅ 提交成功！"); time.sleep(1); # st.rerun()  👈 修正2：這裡加了註解，攔截錯誤！
                    except Exception as e:
                        st.error(f"❌ 寫入失敗: {e}")
        ...
        try:
            ...
            if not pending_df.empty:
                ...
                if c1.button("✅ 核准通過"):
                    ...
                    conn.update(worksheet="Sheet1", data=main_df); st.success("已通過"); st.rerun()
                if c2.button("❌ 執行退回") and audit_note:
                    main_df.at[sel_id, "審核狀態"] = f"退回：{audit_note}"
                    conn.update(worksheet="Sheet1", data=main_df); st.warning("已退回"); st.rerun()
            else: st.success("目前無待審核單")
...
# --- D. 組長最高管理中心 ---
    ...
    if verify_password(input_pw, role):
        ...
        with t1:
            ...
            if st.button("更新公告"): save_system_config(); st.success("公告已更新")
            st.session_state.start_date = st.date_input("開學日", st.session_state.start_date)
            if st.button("更新日期"): save_system_config(); st.success("日期已更新")
        with t2:
        ...
        with t3:
            if not full_df.empty:
                ...
                for i in range(3): st.markdown(f"#### 第 {i+1} 年級"); st.dataframe(week_ranks[i], hide_index=True)
        with t4:
            term_ranks = calculate_leader_stats(full_df[full_df["審核狀態"] == "審核通過"], is_term=True)
            for i in range(3): st.markdown(f"#### 第 {i+1} 年級"); st.dataframe(term_ranks[i], hide_index=True)
